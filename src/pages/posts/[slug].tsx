...
type Props =
  | { ok: true; post: { slug: string; date: string; title: string; excerpt: string; content: string } }
  | { ok: false }
...
export default function PostPage(props: Props) {
  ...
  return (
    <article className="max-w-3xl mx-auto py-10 px-4 prose prose-neutral">
      ...
      <time className="text-sm text-gray-500">{fmt.format(new Date(post.date))}</time>
...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  ...
  const tr = (doc.translations as any)?.[locale] || (doc.translations as any)?.el
